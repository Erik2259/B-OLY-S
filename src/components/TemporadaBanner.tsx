...
export default function TemporadaBanner({ producto, index }: Props) {
  ...
  return (
    <motion.div
      ...
      className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50 border border-orange-100 shadow-card"
    >
      <div className="flex flex-col sm:flex-row">
        ...
        <div className="flex-1 p-5 flex flex-col justify-center">
          ...
          {producto.tipo_producto === 'paquete' && (
            <span className="inline-flex items-center gap-1 bg-orange-100 text-orange-600 text-[10px] font-bold px-2 py-1 rounded-full mt-2 w-fit">
              📦 Paquete completo
            </span>
          )}
          <div className="flex items-center justify-between mt-4">
            ...
            <motion.button
              ...
              className={`flex items-center gap-1.5 text-white text-sm font-bold font-display px-5 py-3 rounded-2xl shadow-md transition-all ${
                justAdded ? 'bg-green-500' : 'bg-gradient-to-r from-boli-orange to-amber-500 hover:from-orange-600 hover:to-amber-600'
              }`}
            >
